...
/*   Updated: 2022/01/09 17:41:49 by pleveque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*crop_line(char *src)
{
	...
	len = ft_strlen_n(src);
	dst = gnl_ft_realloc_cat(&src[len], NULL);
	src = free_null(src);
	if (!dst)
		return (NULL);
	return (dst);
...
char	*get_next_line(int fd)
{
	...
	while (!buffer || (!ft_isline(line)
			&& gnl_ft_strlen(buffer) == BUFFER_SIZE))
	{
		buffer = read_buff(buffer, fd);
		if (!buffer)
			return (free_all(&buffer, &line, &res));
		line = gnl_ft_realloc_cat(buffer, line);
		if (!line)
			return (free_all(&buffer, &line, &res));
	}
	...
	if (!line || !res)
		return (free_all(&buffer, &line, &res));
	if (line && gnl_ft_strlen(line) == 0 && line[0] != '\n')
		line = free_null(line);
	buffer = free_null(buffer);
	return (res);
}
